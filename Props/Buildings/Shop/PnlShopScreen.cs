...
public class PnlShopScreen : Panel
{
    ...
    public void OnItemHovered(IInventoryPlaceable item, PnlInventory source)
    {
        ...
        {
            buyFor = " Hmmph. Trash. I'll take it of your hands for ";
            cost = Convert.ToInt32(Math.Floor(cost/2f));
        }
        else
        {
            buyFor = " A bargain for ";
        ...
        {
            GetNode<Label>("PnlStatus/LblStatus").Text = item.Name + "." + buyFor + cost + (source != GetNode<PnlInventory>("TabContainer/Sell/PnlInventorySell") ? " gold!" : " gold.");
        }
    }
